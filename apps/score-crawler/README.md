...
# 주기 실행
pnpm crawler:loop                             # 기본 300s 간격 루프
...
| 주기 실행 (300s 루프) | `pnpm crawler:loop` | `bash scripts/run.sh --loop --interval-seconds 300` |
...
[score-crawler] targets (3/25): soccer, baseball, basketball

[score-crawler] → soccer (SUPPORTED) internal=football provider=football
[score-crawler]   · url=https://www.livesport.com/kr/soccer/ rows=5
[score-crawler]   · saved matches=5 teams=10
...

[score-crawler] ── summary ──
  sport_mappings:           34 (supported=25)
  crawler_matches_raw:      15
  crawler_teams_raw:        30
```
...
| `--limit-pages` | `3` | 순회할 페이지(종목) 개수 상한 (0 이하면 무제한) |
| `--limit-rows` | `5` | 페이지당 저장할 경기 row 상한 |
...
| `--interval-seconds` | `300` | `--loop` 사이클 간 sleep 초 |
...
# 2) 지원 종목 3개만 5 row 씩 잡고 sqlite 에 저장
python run.py --sports soccer,basketball,tennis --limit-rows 5
