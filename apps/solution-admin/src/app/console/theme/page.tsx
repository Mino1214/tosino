...
export default function ConsoleThemePage() {
  ...
  if (!detail) {
    return <p className="text-zinc-500">테마 정보를 불러오는 중…</p>;
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-100">테마·UI</h1>
          <p className="mt-1 text-sm text-zinc-500">
            브랜드 색·헤더·홈 레이아웃·카드 느낌을 한곳에서 수정합니다. 미리보기는
            브라우저만으로는 서버가 자동으로 켜지지 않습니다 — 아래 명령을{" "}
            <strong className="text-zinc-400">저장소 루트 터미널</strong>에서
            실행하세요.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-lg border border-amber-700/60 bg-amber-950/40 px-4 py-2 text-sm font-medium text-amber-100 hover:bg-amber-900/30 disabled:cursor-not-allowed disabled:opacity-40"
          >
            미리보기 (새 탭)
          </button>
        </div>
      ...
      {postCreateHint && detail.previewPort != null && (
        <div className="rounded-xl border border-amber-800/60 bg-amber-950/30 px-4 py-4 text-sm">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <code className="block max-w-full overflow-x-auto rounded bg-zinc-950 px-3 py-2 font-mono text-xs text-amber-100/90">
              {stackCmd}
            </code>
            <button
              type="button"
              onClick={() => copyPreviewCommand(stackCmd)}
              className="shrink-0 rounded border border-amber-700/50 px-3 py-1.5 text-xs text-amber-200 hover:bg-amber-950/50"
            >
              복사
            </button>
            <button
          ...
          {copyMsg && (
            <p className="mt-2 text-xs text-emerald-400/90">{copyMsg}</p>
          )}
        </div>
      )}
      ...
      {detail.previewPort != null && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-3 text-sm text-zinc-400">
          <p>
            <span className="text-zinc-300">미리보기 포트</span>{" "}
            <code className="rounded bg-zinc-950 px-1.5 py-0.5 text-amber-200/90">
              {detail.previewPort}
            </code>
            <span className="text-zinc-600"> · 슬러그 </span>
            <code className="rounded bg-zinc-950 px-1.5 py-0.5 text-zinc-300">
              {detail.slug}
            </code>
          </p>
          <p className="mt-3 text-xs text-zinc-500">
            <strong className="text-zinc-400">API + 솔루션 한 번에</strong> (권장)
          </p>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <code className="block max-w-full flex-1 overflow-x-auto rounded bg-zinc-950 px-3 py-2 font-mono text-xs">
              {stackCmd}
            </code>
            <button
              type="button"
          ...
          <p className="mt-3 text-xs text-zinc-500">
            <strong className="text-zinc-400">복사본만</strong>{" "}
            <code className="text-zinc-600">solution:provision</code> 은 설치 후{" "}
            <strong className="text-zinc-400">Next가 자동으로 뜹니다</strong>.
            설치만 하려면{" "}
            <code className="rounded bg-zinc-950 px-1 text-zinc-500">
              pnpm solution:provision-only -- {detail.slug} {detail.previewPort}
            </code>
          </p>
          <p className="mt-2 text-xs text-zinc-500">
            <strong className="text-zinc-400">단일 apps/solution-web</strong> 만
            쓸 때:{" "}
            <code className="rounded bg-zinc-950 px-1 text-zinc-500">
          ...
          <p className="mt-2 text-xs text-zinc-600">
            ...
            <code className="text-zinc-500">.env.local</code>에
      ...
      <form
        onSubmit={onSave}
        className="space-y-8 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6"
      >
        {saveMsg && (
          <p
            className={
        ...
        <section className="space-y-4">
          ...
          <label className="block text-sm text-zinc-400">
            ...
            <input
              ...
              className="mt-2 h-10 w-full max-w-[12rem] cursor-pointer rounded border border-zinc-700 bg-zinc-950"
            />
          </label>
          <label className="block text-sm text-zinc-400">
            사이트 이름
